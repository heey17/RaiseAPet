		...
		$.ajax({
			...
			success : function(data){
				if(data=="OK"){
					...
					confirmId.innerHTML = "사용 가능";
				} else{
					confirmId.style.color = "#ff0000";
					confirmId.innerHTML = "사용 중";
		...
		$.ajax({
			...
			success : function(data){
				if(data=="OK"){
					...
					confirmPhone.innerHTML = "사용 가능";
				} else{
					confirmPhone.style.color = "#ff0000";
					confirmPhone.innerHTML = "사용 중";
		...
		$.ajax({
			...
			success : function(data){
				if(data=="OK"){
					...
					confirmEmail.innerHTML = "사용 가능";
				} else{
					confirmEmail.style.color = "#ff0000";
					confirmEmail.innerHTML = "사용 중";
...
<script>
	...
	function emailConfirmNum(){
		...
		output += "<input type='text' id='userPhone' name='userPhone' onblur='phoneOverlap()' onchange='phoneCheck()' placeholder='전화번호' maxlength='11' required='required'>"
		output += "<small><span id='confirmPhone'></span></small><br>";
		output += "<input type='email' id='userEmail' name='userEmail' onblur='emailOverlap()' placeholder='이메일' required='required' maxlength='50'>";
		output += "<small><span id='confirmEmail'></span></small><br>";
		output += "<small><a onclick='emailSender()'>[인증번호 발송]</a></small><br>";
		output += "<input type='text' id='emailConfirmNum' placeholder='인증번호 입력' onchange='emailNumCheck()' required='required' maxlength='5'><br>";
		output += "<small><span id='emailConfirm'></span></small>";
		output += "</div>";
		output += "</td>";
		output += "</tr>";
		$("#area").html(output);
	...
	function phoneConfirmNum(){
		...
		output += "<td>";
		output += "<div class='sertification'>";
		output += "<input type='email' id='userEmail' name='userEmail' onblur='emailOverlap()' onchange='emailCheck()' placeholder='이메일' required='required' maxlength='50'>";
		output += "<small><span id='confirmEmail'></span></small><br>";
		output += "<input type='text' id='userPhone' name='userPhone' onblur='phoneOverlap()' onchange='phoneCheck()' placeholder='전화번호' maxlength='11' required='required'>";
		output += "<small><span id='confirmPhone'></span></small><br>";
		output += "<small><a onclick='phoneSender()'>[인증번호 발송]</a></small><br>";
		output += "<input type='text' id='phoneConfirmNum' placeholder='인증번호 입력' onchange='phoneNumCheck()' required='required' maxlength='6'><br>";
		output += "<small><span id='phoneConfirm'></span></small>";
		output += "</div>";
		output += "</td>";
		output += "</tr>";
		$("#area").html(output);
...
<body>
		...
		<table border=1>			
			<tr>
				<td>
					<form id="form" runat="server">
   						...
   						<img id="image_section" style="height:280px; width: 400px;"/>
					</form>
			...
			<c:choose>
				<c:when test="${naverid ne null}">
					<tr>
						<td>
							* <input type="text" name="userId" id="userId" onblur="idOverlap()" onchange="idCheck()" placeholder="네이버 아이디" required="required" maxlength="15">
							<small><span id="confirmId"></span></small><br>
							<small>영문자, 숫자 혼합하여 15자리 이내(영문자로 시작)</small><br>
							<input type="hidden" name="naverId" id="naverId" value="${naverid}">
							* <input type="password" name="userPw" id="userPw" placeholder="네이버 비밀번호" onchange="pwCheck()" maxlength="15" required="required">
				...
				<c:when test="${kakaoId ne null}">
					<tr>
						<td>
							* <input type="text" name="userId" id="userId" onblur="idOverlap()" onchange="idCheck()" placeholder="카카오 아이디" required="required" maxlength="15">
							<small><span id="confirmId"></span></small><br>
							<small>영문자, 숫자 혼합하여 15자리 이내(영문자로 시작)</small><br>
							<input type="hidden" name="kakaoId" id="kakaoId" value="${kakaoId}">
							* <input type="password" name="userPw" id="userPw" placeholder="카카오 비밀번호" onchange="pwCheck()" maxlength="15" required="required">
				...
				<c:otherwise>
					<tr>
						<td>
							* <input type="text" name="userId" id="userId" placeholder="아이디 " onblur="idOverlap()" onchange="idCheck()" maxlength="15" required="required">
							<small><span id="confirmId"></span></small><br>
							<small>영문자, 숫자 혼합하여 15자리 이내(영문자로 시작)</small><br>
						</td>
					</tr>
					...
					<tr>
						<td>
							* <input type="password" id="userPw" name="userPw" placeholder="비밀번호" onchange="pwCheck()" maxlength="15" required="required">
			...
			<tr>
				<td>
					<!-- 본인인증 화면 전환 start -->
					전화번호 <input type="radio" name="join" onclick="phoneConfirmNum()" checked="checked">
					이메일 <input type="radio" name="join" onclick="emailConfirmNum()"><br>
					<div id="area" class="sertification">	
						<input type="email" id="userEmail" name="userEmail" onblur="emailOverlap()" placeholder="이메일" required="required" maxlength="50">
						<small><span id="confirmEmail"></span></small><br>
						<!-- 본인인증 form start -->
						<input type="text" id="userPhone" name="userPhone" onblur="phoneOverlap()" onchange="phoneCheck()" placeholder="전화번호" maxlength="11" required="required">
						<small><span id="confirmPhone"></span></small><br>
						<small><a onclick="phoneSender()">[인증번호 발송]</a></small><br>
						
						<!-- 본인인증 form end -->
						<input type="text" id="phoneConfirmNum" onchange="phoneNumCheck()" placeholder="인증번호 입력" maxlength="6" required="required"><br>
						<small><span id="phoneConfirm"></span></small>
	...
	<a href="naverjoin"><img width="350" src="resources/img/naverJoin.png"></a><br>
	...
	<img width="350" src="resources/img/kakaoJoin.png"></a>
