...
public class MemberService {
	...
	public ModelAndView memberJoin(MemberDTO member) throws IllegalStateException, IOException {
		...
		String savePath = "D:/DEV/springWork/RaiseAPet/src/main/webapp/resources/memberFile/" + ImageFileName;
	...
	public String buyInfoUpdate(int goodsNum, int point, int couponCode, int payCheck, int total_pay, int addressCheck) {
		...
		MemberDTO memberSession = mDAO.memberSession(map);
		
		session.setAttribute("loginUser", memberSession);
	...
	public String selectBuyInfoUpdate(int goodsNum2, int addressCheck, int point, int total_pay, int payCheck, int couponCode) {
		...
		System.out.println("goodsNum2 : " + goodsNum2);
		...
		// ------------- 포인트 차감 후 회원 정보 조회 -------------
		MemberDTO memberInfo = mDAO.memberSession(map);
		...
		if(memberInfo != null && memberPointUpdate > 0) {
			resultMsg = "OK";
		}else {
			resultMsg = "NO";
		}
