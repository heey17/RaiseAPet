...
// 게시판 관리
	...
	public ModelAndView hotelBoardWrite(HotelDTO hotel, List<MultipartFile> fileList) throws IllegalStateException, IOException {
		...
		//파일 저장 경로
        String path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+date.format(time)+"ㅡ";
	...
	public ModelAndView hotelBoardModifyProcess(HotelDTO hotel, List<MultipartFile> fileList) throws IllegalStateException, IOException {
		// 게시글 수정 전 서버에 저장된 사진 삭제
				...
				String deletePath = "";
				deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotelResult.getHotelRoomImage1(); // 삭제할 파일의 경로
				file = new File(deletePath);
				if(file.exists() == true){
					file.delete();
				}
				deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotelResult.getHotelRoomImage2(); // 삭제할 파일의 경로
				file = new File(deletePath);
				if(file.exists() == true){
					file.delete();
				}
				deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotelResult.getHotelRoomImage3(); // 삭제할 파일의 경로
				file = new File(deletePath);
				if(file.exists() == true){
					file.delete();
				}
				deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotelResult.getHotelRoomImage4(); // 삭제할 파일의 경로
				file = new File(deletePath);
				if(file.exists() == true){
					file.delete();
				}
				deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotelResult.getHotelRoomImage5(); // 삭제할 파일의 경로
				file = new File(deletePath);
				if(file.exists() == true){
					file.delete();
				}
	...
	public ModelAndView hotelBoardDelete(String hotelCode) {
		...
		String path = "";
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotel.getHotelRoomImage1(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotel.getHotelRoomImage2(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotel.getHotelRoomImage3(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotel.getHotelRoomImage4(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/hotelFile/"+hotel.getHotelRoomImage5(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
	...
	public ModelAndView beautyBoardWrite(BeautyDTO beauty) throws IllegalStateException, IOException {
		...
		// 파일 저장 경로
		String savePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/beautyFile/"+fileName;
	...
	public ModelAndView beautyBoardDelete(String designerCode) {
		...
		String path = "";
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/beautyFile/"+beauty.getDesignerImage(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
	...
	public ModelAndView medicalBoardWrite(MedicalDTO medical) throws IllegalStateException, IOException {
		...
		// 파일 저장 경로
		String savePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/medicalFile/"+fileName;
	...
	public ModelAndView medicalBoardDelete(String doctorCode) {
		...
		String path = "";
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/beautyFile/"+medical.getDoctorImage(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
	...
	public ModelAndView educationBoardWrite(EducationDTO education) throws IllegalStateException, IOException {
		...
		// 파일 저장 경로
		String savePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/educationFile/"+fileName;
	...
	public ModelAndView educationBoardDelete(int eduNum) {
		...
		String path = "";
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/beautyFile/"+education.getEduImage(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
	...
	public ModelAndView goodsBoardWrite(GoodsDTO goods, List<MultipartFile> fileList) throws IllegalStateException, IOException {
		//파일 저장 경로
        String path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+date.format(time)+"ㅡ";
	...
	public ModelAndView goodsBoardModifyProcess(GoodsDTO goods, List<MultipartFile> fileList) throws IllegalStateException, IOException {
		...
		GoodsDTO goodsResult=adminDAO.goodsBoardView(goods.getGoodsNum());
						String deletePath = "";
						deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goodsResult.getGoodsImage1(); // 삭제할 파일의 경로
						file = new File(deletePath);
						if(file.exists() == true){
							file.delete();
						}
						deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goodsResult.getGoodsImage2(); // 삭제할 파일의 경로
						file = new File(deletePath);
						if(file.exists() == true){
							file.delete();
						}
						deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goodsResult.getGoodsImage3(); // 삭제할 파일의 경로
						file = new File(deletePath);
						if(file.exists() == true){
							file.delete();
						}
						deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goodsResult.getGoodsImage4(); // 삭제할 파일의 경로
						file = new File(deletePath);
						if(file.exists() == true){
							file.delete();
						}
						deletePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goodsResult.getGoodsImage5(); // 삭제할 파일의 경로
						file = new File(deletePath);
						if(file.exists() == true){
							file.delete();
						}
	...
	public ModelAndView goodsBoardDelete(int goodsNum) {
		...
		String path = "";
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goods.getGoodsImage1(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goods.getGoodsImage2(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goods.getGoodsImage3(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goods.getGoodsImage4(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/goodsFile/"+goods.getGoodsImage5(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
	...
	public ModelAndView eventBoardWrite(EventDTO event) throws IllegalStateException, IOException {
		...
		// 파일 저장 경로
		String savePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/eventFile/"+fileName;
		String saveTitlePath = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/eventFile/"+titleFileName;
	...
	public ModelAndView eventBoardDelete(int eventNum) {
		...
		String path = "";
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/beautyFile/"+event.getEventImage(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
		path = "D:/SPRINGWORK/RaiseAPet/src/main/webapp/resources/beautyFile/"+event.getEventTitleImage(); // 삭제할 파일의 경로
		file = new File(path);
		if(file.exists() == true){
			file.delete();
		}
